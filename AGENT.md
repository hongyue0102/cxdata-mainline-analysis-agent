...
若 fetch_data.py 输出含 `confirmation_required`（本轮会话已成功调用 50 次计费接口），**必须暂停**，向用户说明：

> 本轮会话已成功调用 50 次计费接口，继续调用消耗积分可能超出预期，是否继续？

用户明确确认后，执行：

```bash
python3 query.py session confirm
```

然后重新运行 fetch_data.py 继续。用户拒绝或未确认时停止调用接口。
...
6. **一句话AI结论**：基于以上数据给出结构性观点，**不得给出买卖指令**（详见上方禁止事项第8条）
7. **免责声明**：报告末尾必须附加免责声明（详见 Step 5 模板）
